export const COLOR_BLACK = 0x000000;
export const COLOR_WHITE = 0xffffff;
export const COLOR_YELLOW = 0xffff00;
export const COLOR_ORANGE = 0xf57f17;
export const COLOR_DARK_ORANGE = 0xbc5100;
export const COLOR_DARK_GREY = 0x333333;

export const COLOR_STRING_BLACK = "#000";
export const COLOR_STRING_WHITE = "#fff";
export const COLOR_STRING_DARK_ORANGE = "#bc5100";
export const COLOR_STRING_DARK_GREY = "#333333";

export const TINT_GREEN = 0x7cfc00;
export const TINT_ORANGE = 0xfc8000;
export const TINT_BLUE = 0x007cfc;
export const TINT_PURPLE = 0x8000fc;
export const EXTERNAL_TINTS = [TINT_ORANGE, TINT_BLUE, TINT_PURPLE];

export const COLOR_MODAL_BACKGROUND = COLOR_BLACK;
export const OPACITY_MODAL_BACKGROUND = 0.7;
export const COLOR_DIALOG_BACKGROUND = COLOR_ORANGE;
export const COLOR_DIALOG_FOREGROUND = COLOR_DARK_ORANGE;
